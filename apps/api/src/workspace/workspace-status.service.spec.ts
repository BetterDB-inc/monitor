...
describe('WorkspaceStatusService', () => {
  ...
  it('keeps counting while the workspace is not bootstrapped', async () => {
    ...
    const service = new WorkspaceStatusService(config, authWithUsers(count), new BootstrapLock());
  ...
  it('does not latch an owner that a failed first sign-in discards', async () => {
    const lock = new BootstrapLock();
    const count = jest.fn().mockResolvedValue(1);
    const service = new WorkspaceStatusService(config, authWithUsers(count), lock);
    let discardOwner: () => void = () => {
      return undefined;
    };
    const failedSignIn = lock.run(() => {
    ...
    expect((await status).bootstrapped).toBe(false);
  ...
  it('reports the broker as enabled in both the fresh and the bootstrapped branch', async () => {
    ...
    const count = jest.fn().mockResolvedValue(1);
    const service = new WorkspaceStatusService(enabled, authWithUsers(count), new BootstrapLock());
  ...
  it('reports the broker as off when AUTH_BROKER_DISABLED is set', async () => {
    ...
    const count = jest.fn().mockResolvedValue(0);
    const service = new WorkspaceStatusService(disabled, authWithUsers(count), new BootstrapLock());
