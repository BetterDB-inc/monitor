...
| `TRUST_PROXY` | No | `false` | Trust `X-Forwarded-For`, `X-Forwarded-Host` and `X-Forwarded-Proto`. Set to `true` behind a reverse proxy so client IPs, rate limits and the CSRF origin check see the browser-facing host, or to a hop count (`1`) or a comma-separated list of proxy addresses/CIDRs to trust only those. Leave unset when the API is reachable directly: the headers are then client-supplied |
