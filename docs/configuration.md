...
**Activity log**: Every `POST`, `PUT`, `PATCH`, and `DELETE` request made by a signed-in user is recorded with the user, the source IP, the connection (`X-Connection-Id`), the route, the response status and, where the route has one, the affected object (a connection id, an invitation id, a member id, a bulk-delete job id). Sign-ins, sign-outs and invitation acceptances are recorded as `auth.login` / `auth.logout`, and every browser CLI command as `cli.command` with the command name and argument count (argument values are kept only for read commands). Requests the server refuses with `401` or `403` are not recorded. Admins read the log from the Activity tab of the Team page or from `GET /api/workspace/activity` (`actor`, `action`, `from`, `to`, `cursor`, `limit` ≤ 100), and rows are kept for `ACTIVITY_RETENTION_DAYS`.
