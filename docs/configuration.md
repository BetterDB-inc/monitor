...
| `AUTH_PUBLIC_URL`         | No       | inferred               | Public origin used for session cookies and CSRF origin checks, e.g. `https://monitor.example.com`. When unset, the origin check accepts the host the request carries under either scheme; setting it pins the check to this origin. Browser CLI and monitor tail WebSocket upgrades that carry an `Origin` header are accepted only from this origin, from the host the request carries, or, outside production (`NODE_ENV` other than `production`), from the Vite dev server at `http://localhost:5173`, and answer `403` otherwise. Session cookies are marked `Secure` only when this starts with `https://`, so installs served over HTTPS (including behind a TLS-terminating proxy) should set it and keep session cookies off plain HTTP |
...
| `AUTH_BROKER_URL`         | No       | `https://betterdb.com` | Origin of the BetterDB sign-in page for Google and GitHub. In production (`NODE_ENV=production`) broker sign-in only turns on once `AUTH_PUBLIC_URL` is also set, because the callback origin must not come from the request's `Host` header                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `AUTH_BROKER_DISABLED`    | No       | `false`                | Turn off Google and GitHub sign-in, e.g. for air-gapped installs. The buttons are hidden and the broker sign-in routes answer `404`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `AUTH_BROKER_PUBLIC_KEY`  | No       | embedded key           | PEM public key that verifies Google and GitHub sign-ins. Replaces the embedded key. The embedded key ships with the release that turns the feature on; until then, Google and GitHub sign-in stays off unless this is set. Write line breaks as `\n` to keep the key on one line                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `AUTH_BROKER_KEY_ID`      | No       | `brk-override`         | Key id for the `AUTH_BROKER_PUBLIC_KEY` override                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
**Roles**: The first account to register becomes the workspace owner, who holds the `admin` role. Admins can change anything. Members can see the team's names and roles but not email addresses. Members are read-only: every `POST`, `PUT`, `PATCH`, and `DELETE` request answers `403` except signing in/out and a small set of read-only `POST` endpoints (vector-search similarity search, profiling, and full-text search, plus the monitor session preflight check, which run a query but do not mutate data). Two reads are admin-only as well: `GET /metrics/config` and `GET /metrics/config/:parameter` answer `403` for members because the server configuration they return can carry credentials (for example `requirepass` or `masterauth`). The browser CLI and the monitor tail both require a signed-in session — an unauthenticated socket is rejected before the WebSocket handshake completes — and a member's CLI session is restricted to read-only commands regardless of `BETTERDB_UNSAFE_CLI`; `CONFIG`, `ACL`, `DEBUG`, and `CLIENT` are refused for members outright because they expose server configuration and credentials, and `SLOWLOG RESET`, `COMMANDLOG RESET`, and `LATENCY RESET` are refused because they clear diagnostic state. Members can read monitor capture sessions, including the live tail; captured values are redacted only when `MONITOR_REDACT_VALUES=true`.

**Sign in with Google or GitHub**: Self-hosted installs can offer Google and GitHub sign-in buttons on the login, register and accept-invite screens, handled by a sign-in page at `AUTH_BROKER_URL`. The install needs outbound browser access to that origin; nothing else leaves your network. Signing in with Google or GitHub only works for an email that already belongs to a member, has a pending invitation, or arrives while the workspace is empty (in which case that person becomes the owner) — linking a Google or GitHub account to a monitor account is always by matching email. In production, this requires `AUTH_PUBLIC_URL` to be set: without it, `GET /system/workspace` reports the buttons as unavailable and the sign-in routes answer `404`, because the callback address sent to the broker must come from a configured origin, not from whatever `Host` header the request happened to carry. The key that verifies these sign-ins ships with the release that turns the feature on. Until then, Google and GitHub sign-in stays off unless you set `AUTH_BROKER_PUBLIC_KEY`. `AUTH_BROKER_DISABLED=true` hides the buttons and makes the sign-in routes answer `404`.

**Invitations**: Admins add people from the Team page. Creating an invitation returns a one-time link (`<AUTH_PUBLIC_URL>/invite/<token>`, or the requesting origin when `AUTH_PUBLIC_URL` is unset) that the admin copies and sends by any channel; the monitor sends no email. The link is shown once, expires after 7 days, and is stored only as a hash. Opening it shows the invited email and role; the invitee sets a name and password and is signed in as a `member` or `admin` according to the invitation. Admins can revoke pending invitations; re-inviting an email whose earlier invitation was revoked, accepted or expired issues a fresh link. The owner can change any other member's role, remove members (which ends their sessions), and transfer ownership to an admin (a member must first be promoted to admin); the previous owner then remains an admin. Public registration stays closed after the first account.
...
| `PROMETHEUS_STALENESS_MS`      | No       | 3 × poll interval       | Drop a connection's gauge series after this long without a successful `INFO` read (ms, ≥ 1000, raised to 2 × poll interval if lower) |
...
| `/metrics/config`                     | GET    | Configuration values matching pattern (admin only) |
| `/metrics/config/:parameter`          | GET    | Value of specific config parameter (admin only)    |
